...
- 分工模式改用固定版 Ruflo 3.43.0 作为任务、Agent、依赖、重试、消息与项目记忆的控制面；Codex Agent 继续在隔离工作区执行实际文件操作。
- 新增首次使用安装与健康检查向导、按项目隔离的 Ruflo MCP 运行目录、崩溃重连与恢复对账、项目私有记忆和用户确认后的通用经验发布。
- Mission schema v2 记录 swarm、动态角色、实际模型、模型原因、Ruflo task/agent ID、运行时健康和同步状态；旧版 Mission 保持只读并可复制为 Ruflo 任务。
- 任务中心新增团队消息、记忆命中、运行时诊断和高级 Ruflo 详情。

### Changed

- 新 Mission 使用 hierarchical、specialized、raft 与项目隔离的 hybrid memory；Ruflo 不可用时暂停并引导修复，不再回退旧分工引擎。
- 模型选择支持自动模式与锁定模式，任务卡同时显示动态角色、实际模型和选择原因。
- 自动模型模式会采用 Ruflo 路由建议并映射到本机可用模型；用户锁定模型始终优先，选择来源和原因写入任务记录。
- Ruflo 内存初始化禁止隐式联网下载模型，优先复用受控缓存并在不可用时立即回退本地向量实现，避免首次任务长时间卡住。
- 工作者报告失败时禁止被缺省阶段审核误判为接受：首次失败进入一次重试，重试后仍失败则明确终止节点。

### Validation

- 新增 MCP 初始化、能力发现、JSON-RPC、任务映射、敏感字段脱敏和项目记忆隔离测试，并保留原有 Mission、对话、附件、任务中心与可靠性回归。
