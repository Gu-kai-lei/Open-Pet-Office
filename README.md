...
**v0.15.0 · 2026-09-26（源码已合并，portable 候选构建）**

- 🧠 新分工任务由固定版 Ruflo 3.43.0 管理团队、任务依赖、消息和项目记忆；Codex Agent 继续在隔离工作区执行文件操作
- 🧭 首次开启分工提供安装与健康检查，计划页展示角色、依赖、模型选择原因和记忆命中；任务中心可查看运行状态与诊断
- 🔄 新版 Mission 支持运行时对账、重试与恢复；旧任务可查看并复制为 Ruflo 任务
- ✅ `npm test` 与 Ruflo Windows MCP 合约测试通过，源码和 portable 均已完成 Electron 界面截图检查

当前候选构建的真实工作者写文件验收仍受本机 Codex Windows 沙箱 ACL 故障阻碍，尚未达到正式发布门槛。详情见 [更新日志](CHANGELOG.md)。
